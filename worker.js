...
export default {
  async fetch(request, env, ctx) {
        ...
        if (path === '/admin/schemas' && method === 'POST') {
          ...
          try {
            ...
            if (error instanceof SyntaxError) {
              return new Response(JSON.stringify({ error: 'Invalid JSON schema' + ":" + error }), { status: 400, headers: { 'Content-Type': 'application/json' } });
            }
            return new Response(JSON.stringify({ error: 'Failed to create schema' + ":" + error }), { status: 500, headers: { 'Content-Type': 'application/json' } });
          }
        }
    ...
    // 静态文件服务 - 检查是否为静态资源路径
    if (path.startsWith('/public/') || path === '/' || ['.html', '.css', '.js', '.json', '.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.txt', '.pdf'].some(ext => path.endsWith(ext))) {
      try {
        // 构造静态资源的路径
        const assetPath = path === '/' ? '/index.html' : path;
        const assetRequest = new Request(`https://example.com${assetPath}`, request);

        // 尝试通过 ASSETS 绑定获取静态文件
        if (env.ASSETS) {
        ...
        // 如果 ASSETS 绑定不可用，继续执行其他路由
      }
    }

    // 根据路径提供不同的默认响应
    if (path === '/') {
      return new Response('Dynamic Schema API Server is running', { status: 200, headers: { 'Content-Type': 'text/plain' } });
    }
